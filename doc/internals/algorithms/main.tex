...
Every event in the unfolding should have a list of events which are in conflict with it. Whenever 
The only case that we need to check direct conflict between two events is when adding a new event $e$ to a configuration C. 
By the time of adding $e$ to C, we need to check if there exist any $e'$ in en(C) which conflicts with e. If 'yes', move $e'$ to cex(C) and update their direct conflict set at the same time.

It is the fact that $e$ and $e'$ are both in en(C), so we have:
\begin{itemize}
	\item
	...
	\item
		$\lceil e' \rceil \cup e$ is a configuration.
\end{itemize} 
To decide if they are in conflcit, just check their parents together. If there is at least one shared parent between them, they are not in direct conflict. Otherwise, they are not.
...
\begin{itemize}
	\item
		If either $e$ or $e'$ is a LOC: two events definitly share no parent (thank to the deterministic 				feature of the model) which means they are not in conflict (including direct conflict).
		
	\item
		If they are two RDs or a RD and a SYN: If \verb!e.pre_mem = e'.pre_mem!, they are not in conflict
		but concurrent. Therefore, if we have \verb!e.pre_proc = e'.pre_mem! 
		or \verb!e.pre_mem = e'.pre_proc!, $e$ and $e'$ are in direct conflict.
	...
	\item
		...
		\begin{itemize}
			...
			\item
				For each \verb!e'.pre_readers!, 
				check \verb!e'.pre_readers[i] = e.pre_proc! or \verb!e'.pre_readers[i] = e.pre_mem!
		\end{itemize}
	\item
		If both are WRs, check each parent of $e$ with those of $e'$. If there exist a shared parent, $e$ and $e'$ conflict directly.
\end{itemize}

\begin{algorithm}{}
Given two events $e$ and $e'$, do as follows to check the direct (immediate) conflict between two events:
\begin{enumerate}
	\item
		If there is no share parent, they are not in direct conflict.
	\item
		If two events share at least one parent (which can be \verb!pre_proc!,\verb!pre_mem! or one of 					\verb!pre_readers! ), we have some different cases:
		\begin{itemize}
			\item
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item 
					...
					\begin{itemize}
					\item
						Formally, \verb!var_maxevt! $ = (e_1, e_2...,e_m)$  (m is the number of variables in the model)
