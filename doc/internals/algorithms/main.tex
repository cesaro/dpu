...
In Event class, there is a vector \verb!maxevt! $maxevt = (e_1, e_2...,e_n)$ (n is the number of global variables) which stores pointers to maximal events for all global variables $v\_i$.
...
\item
	...
	\item
		\verb!e.maxevt(x) = e!
	\item	
		$\forall y \neq x$: \verb!e.maxevt(y) = max(e.pre_proc.maxevt(y), e.pre_mem.maxevt(y) )!
	\end{itemize}
\item
	If e is a WR: 
	\begin{itemize}
	\item
		\verb!e.maxevt(x) = e!
	\item	
		$\forall y \neq x$: \verb! e.maxevt(y) = max(e.pre_proc.maxevt(y), max(e.pre_readers[i])(y))!
		$pre\_readers[i]$ is the previous reader corresponding to process i 
...
\begin{itemize}
\item
	The first element of the list \verb!skip_preds[0]! indicates the predecessor which is in a depthh distance of $k^0 = 1$. It is actually \verb!pre_proc! or \verb!pre_mem! of corresponding event.
\item
