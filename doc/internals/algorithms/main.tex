...
\begin{enumerate}
	...
	\item e and e' are in different processes: $p(e)\neq p(e')$
		\begin{itemize}
			...
			\item touch the same variable: $v(e) = v(e')$ and they are (RD, RD). Assume $WR_1$, $WR_2$ are the latest WR events in two local configurations of $e$ and $e'$ respectively. Apply \cref{a:layer_tree} to determine the relation between two WRs: they are either in conflict or in causality.
			\begin{itemize}
				\item
					$WR_1 \cfl WR_2$: we have $e \cfl e'$.  
				\item
					$WR_1 < WR_2$, then $e < e'$ or $e \cfl e'$. To determine the case, 
				
			\end{itemize}
			 apply the \cref{a:reads}
			\item 
				touch the same variable: $v(e) = v(e')$ and they are a pair of (RD,WR).
				
			\item Other cases where two events touching different variables: $v(e) \neq v(e')$) or two LOCs, store more information:	
			\begin{itemize}
				...
				and $e_i < e_i'$ .
				\item Apply \cref{a:layer_tree} to check conflict for all pairs of events $(e_i, e_i')$ with $e_i \in e.maxevt, e_i'\in e'.maxevt$
			\end{itemize}	
		\end{itemize}
\end{enumerate}
