...
Tree are divided into layers where events have the same depth and are in immediate conflict.
e and e' are in the same tree at different depths, assume depth(e) >depth(e'). If e' is one of e's causal predecessors, they are in causality, else they are in conflict.
To quickly go through the tree, we use idea of skip list(see \ref{fig:trees}. Accordingly, a list of event's skip predecessors $ skip_preds = (i_1, i_2,..,i_m)$ are stored, where $i_j$ $(j \in [1,m] )$ is the node's predecessor in $k^{j}$ distance of depth, k (k > 1) is a constant choosen to skip.

\verb!Initialize a list of skip predecessors! $skip_preds$:
e is new event added to the unfolding. Firstly, we need to define the size of vector $skip\_preds$
\begin{itemize}
\item 
...
\begin{itemize}
	...
	\item \textit{skip\_pres}: list of its k-skip predecessors.
