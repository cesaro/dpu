...
\item
	...
	\item
		...
		J will be the union of all their local configuration: $ J = \cup [e'_i]$. Condition (2) is satisfied.
	\end{itemize}
\end{itemize}
...
To quickly find $e"$ through the tree, we use the idea of skip list (see \ref{fig:trees}. Accordingly, in each node, a list of skip predecessors $ skip_preds = (i_1, i_2,..,i_m)$ are stored, where $i_j$ $(j \in [1,m] )$ is the node's predecessor in $k^{j}$ distance of depth, k (k > 1) is a constant distance in depth choosen to skip.
...
\begin{itemize}
	...
	\item \textit{skip\_pres}: list of pointers to its k-skip predecessors of type T.	
(T can be an event in the same process or touching the same variable)
\end{itemize}

To apply the algorithm of skip list to both tree of events in the same process and those touching the same variable, we use a template class \verb!MultiNode<T,S>!(T is Event and S is an integer) which has the following attributes:
\begin{itemize}
...
\end{itemize}
 ...
 Class Event inherits \verb!MultiNode<T,S>! and has two template function indicating which type of Node to take into account.
\begin{itemize}
\item	
	\verb!Node<Event> &proc!: return node[0] equivalant to variable tree
\item	
	\verb!Node<Event> &var!: return node[1] equivalant to variable tree 	
\end{itemize}

\noindent
\textbf{Initialize a list of skip predecessors} $skip\_preds$:
Firstly, we need to define the size of vector $skip\_preds$ of a node
\begin{itemize}
\item 
	If $(e.depth) \% k \neq 0$, size($e.skip\_pres$) is 1.
\item 
	If $(e.depth) \% k = 0 $, compute $m = \left[ log_k(e.depth)\right]$ (floor), size of list is $s \in [1,m]: e.depth \% kˆ{s} = 0 $ 
\end{itemize}
After that, add all elements.
\begin{itemize}
\item
	If $m = log_k(e.depth) \in \N$, repeate $e' = e \rightarrow pre$ k times.
	Assign $e.skip\_pres[0] = e'$
\end{itemize}
$e.skip\_preds[i]$ is a predecessor which is in a depth distance of $k^{j}$ with e.
\begin{itemize}
\item
	$e.skip\_preds[0]$ is its immediate predecessor is $pre\_proc$ (in case, it is tree of events in the same process) or $pre\_mem$ (in 			case, it is tree of events touching the same variable).
\item 
	$e.skip\_preds[1]$ is the predecessor which is found by going back k times with the pointer $pre\_proc$ or $pre\_mem$
\end{itemize}


\begin{algorithm}
\noindent
 Assume e.depth > e'.depth, d = e.depth - e'.depth, do as follows:
\begin{enumerate}
\item
