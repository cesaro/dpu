...
/*
 * This program is the blkiomon program from the package blktrace-1.0.5. We have
 * slightly modified it to run in on the DPU verifier. All modified lines are
 * next to a comment containing the string "dpu".
 */
...
// dpu, limits to the two infinite loops in the program
static int main_loop_count = INT_MAX;
static int thread_loop_count = INT_MAX;
...
static void *blkiomon_interval(void *data)
{
	...
	while (1) {
      if (--thread_loop_count == 0) break; // dpu
...
static int blkiomon_do_fifo(void)
{
	...
	while (up) {
      ...
      if (--main_loop_count == 0) break; // dpu
	}
...
// dpu, new options -i, -x, -y
#define S_OPTS "b:d:D:h:I:Q:q:m:Vi:x:y:"
...
static char usage_str[] = "\n\nblkiomon " \
	...
	"[ -V                | --version ]\n\n" \
	"\t-i   input file, instead of standard input (for DPU).\n" \
	"\t-x   Max number of iterations of main loop (for DPU).\n" \
	"\t-y   Max number of iterations of thread loop (for DPU).\n" \
	"\t-I   Sample interval.\n" \
...
void blkiomon_signal(int signal)
{
...
int main(int argc, char *argv[])
{
   ...
   // dpu, we do not support signal handling
	...
	while ((c = getopt_long(argc, argv, S_OPTS, l_opts, NULL)) != -1) {
		...
		case 'i': // dpu
			...
			break;
		case 'x': // dpu
			main_loop_count = atoi (optarg);
			break;
		case 'y': // dpu
			thread_loop_count = atoi (optarg);
			break;
		case 'V':
   ...
   // dpu, we read from a hard file rather than the standard input
	//ifp = fdopen(STDIN_FILENO, "r");
   ifp = fopen (input_path, "r");

	if (!ifp) {
		perror("blkiomon: could not open 'input' for reading");
		return 1;
	}
   ...
   // dpu, we wait for the other thread to finish
   pthread_exit(0);
