...
inline void Unfolder::report_add_nondet_violation (const Trail &t, unsigned where, ActionType found)
{
   Defect d;
...
bool Unfolder::stream_match_trail
...
bool Unfolder::stream_to_events
      (Config &c, const stid::action_streamt &s, Trail *t, Disset *d)
{
   ...
   DEBUG ("unf: s2e: c %s t %zd", c.str().c_str(), t ? t->size() : -1);
